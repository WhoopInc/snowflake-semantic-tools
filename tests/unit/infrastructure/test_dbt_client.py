...
from snowflake_semantic_tools.infrastructure.dbt import DbtClient, DbtCompileError, DbtNotFoundError, DbtResult
...
class TestDbtTypeDetection:
    ...
    def test_detects_pip_cloud_cli(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = Mock(stdout="dbt Cloud CLI - 0.40.7 (3aa8c1ef 2025-10-06)", stderr="", returncode=0)
    ...
    def test_detects_dbt_core(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = Mock(stdout="Core:\n  - installed: 1.7.15", stderr="", returncode=0)
    ...
    def test_defaults_to_core_on_unrecognized_output(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = Mock(stdout="some unknown dbt version 3.0", stderr="", returncode=0)
...
class TestDbtCompileCommand:
    ...
    def test_compile_with_target_for_core(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            mock_run.return_value = Mock(stdout="Core:\n  - dbt-core: 1.7.0", stderr="", returncode=0)
            client = DbtClient()
    ...
    def test_compile_without_target_for_cloud_cli(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            mock_run.return_value = Mock(stdout="dbt Cloud CLI - 0.40.7", stderr="", returncode=0)
            client = DbtClient()
    ...
    def test_compile_success_returns_result(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            mock_run.return_value = Mock(stdout="Compiled 10 models", stderr="", returncode=0)
    ...
    def test_compile_failure_returns_result_with_error(self):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            mock_run.return_value = Mock(stdout="", stderr="Compilation error: model not found", returncode=1)
            result = client.compile()
...
class TestDbtResult:
    ...
    def test_result_has_all_fields(self):
        ...
        result = DbtResult(
            success=True, command="dbt compile", stdout="output", stderr="", returncode=0, dbt_type=DbtType.CORE
        )
...
class TestEdgeCases:
    ...
    def test_handles_version_with_only_stderr(self):
        ...
        with patch("subprocess.run") as mock_run:
            mock_run.return_value = Mock(stdout="", stderr="Core:\n  - dbt-core: 1.7.0", returncode=0)
