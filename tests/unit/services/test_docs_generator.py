...
class TestDocsGeneratorHTML:
    ...
    def test_index_contains_search_overlay(self, tmp_path, manifest, graph):
        gen = DocsGenerator(manifest, graph)
        gen.generate(DocsConfig(output_dir=tmp_path / "docs"))
        html = (tmp_path / "docs" / "index.html").read_text()
    ...
    def test_index_contains_lineage_view(self, tmp_path, manifest, graph):
        gen = DocsGenerator(manifest, graph)
        gen.generate(DocsConfig(output_dir=tmp_path / "docs"))
        html = (tmp_path / "docs" / "index.html").read_text()
        assert "view-lineage" in html
        assert "lineage-container" in html
        assert "GRAPH_DATA" in html

    def test_index_contains_d3_and_dagre(self, tmp_path, manifest, graph):
        gen = DocsGenerator(manifest, graph)
        gen.generate(DocsConfig(output_dir=tmp_path / "docs"))
        html = (tmp_path / "docs" / "index.html").read_text()
    ...
    def test_index_contains_sidebar(self, tmp_path, manifest, graph):
        gen = DocsGenerator(manifest, graph)
        gen.generate(DocsConfig(output_dir=tmp_path / "docs"))
        html = (tmp_path / "docs" / "index.html").read_text()
    ...
    def test_html_has_detail_panel(self, tmp_path, manifest, graph):
        gen = DocsGenerator(manifest, graph)
        gen.generate(DocsConfig(output_dir=tmp_path / "docs"))
        html = (tmp_path / "docs" / "index.html").read_text()
    ...
    def test_html_is_self_contained(self, tmp_path, manifest, graph):
        gen = DocsGenerator(manifest, graph)
        gen.generate(DocsConfig(output_dir=tmp_path / "docs"))
        html = (tmp_path / "docs" / "index.html").read_text()
