...
def _dbt_manifest(nodes=None, sources=None):
    return {
...
class TestDbtLineageIntegration:
    def test_dbt_sources_added(self):
        sst = _minimal_manifest(tables=[{"table_name": "orders"}])
        dbt = _dbt_manifest(
            nodes={
                "model.proj.orders": {
                    "resource_type": "model",
        ...
        graph = LineageGraphBuilder.from_manifest_data(sst, dbt_manifest=dbt)
        source_nodes = [n for n in graph.nodes.values() if n.type == "dbt_source"]
        assert len(source_nodes) == 1
        assert source_nodes[0].name == "raw.raw_orders"

    def test_dbt_staging_models_added(self):
        sst = _minimal_manifest(tables=[{"table_name": "orders"}])
        dbt = _dbt_manifest(
            nodes={
        ...
        graph = LineageGraphBuilder.from_manifest_data(sst, dbt_manifest=dbt)
        dbt_models = [n for n in graph.nodes.values() if n.type == "dbt_model"]
        assert len(dbt_models) == 1
        assert dbt_models[0].name == "stg_orders"

    def test_dbt_depends_on_edges(self):
        sst = _minimal_manifest(tables=[{"table_name": "orders"}])
        dbt = _dbt_manifest(
            nodes={
                "model.proj.stg_orders": {
        ...
        graph = LineageGraphBuilder.from_manifest_data(sst, dbt_manifest=dbt)
        depends_edges = [e for e in graph.edges if e.edge_type == "depends_on"]
        assert len(depends_edges) >= 2
        table_deps = [e for e in depends_edges if e.source_id == "table:ORDERS"]
        assert len(table_deps) == 1
        assert table_deps[0].target_id == "dbt_model:STG_ORDERS"

    def test_dbt_full_pipeline(self):
        sst = _minimal_manifest(
            tables=[{"table_name": "orders"}],
            metrics=[{"name": "total", "tables": ["orders"], "expr": "COUNT(*)"}],
            semantic_views=[{"name": "sales", "tables": ["orders"]}],
        )
        dbt = _dbt_manifest(
            nodes={
                "model.proj.stg_orders": {
        ...
        graph = LineageGraphBuilder.from_manifest_data(sst, dbt_manifest=dbt)
        upstream = graph.get_upstream("semantic_view:SALES")
        upstream_ids = {n.id for n in upstream}
        assert "table:ORDERS" in upstream_ids
        downstream = graph.get_downstream("dbt_source:SOURCE.PROJ.RAW.RAW_ORDERS")
        downstream_ids = {n.id for n in downstream}
        assert "dbt_model:STG_ORDERS" in downstream_ids

    def test_no_dbt_manifest_no_dbt_nodes(self):
        sst = _minimal_manifest(tables=[{"table_name": "orders"}])
        graph = LineageGraphBuilder.from_manifest_data(sst, dbt_manifest=None)
        dbt_nodes = [n for n in graph.nodes.values() if n.type in ("dbt_source", "dbt_model")]
        assert len(dbt_nodes) == 0

    def test_sst_tables_not_duplicated_as_dbt_models(self):
        sst = _minimal_manifest(tables=[{"table_name": "orders"}])
        dbt = _dbt_manifest(
            nodes={
                "model.proj.orders": {
...
class TestLayerComputation:
    def test_layers_by_type(self):
        ...
        graph = LineageGraphBuilder.from_manifest_data(manifest)
        d3 = graph.to_d3_json()
        node_layers = {n["id"]: n["layer"] for n in d3["nodes"]}
        assert node_layers["table:ORDERS"] == 2
        assert node_layers["metric:TOTAL"] == 3
        assert node_layers["semantic_view:V1"] == 4

    def test_d3_json_includes_layer(self):
        manifest = _minimal_manifest(tables=[{"table_name": "t1"}])
        graph = LineageGraphBuilder.from_manifest_data(manifest)
        d3 = graph.to_d3_json()
        assert all("layer" in n for n in d3["nodes"])

    def test_dbt_source_layer_zero(self):
        sst = _minimal_manifest(tables=[{"table_name": "orders"}])
        dbt = _dbt_manifest(
            nodes={
                "model.proj.orders": {
        ...
        graph = LineageGraphBuilder.from_manifest_data(sst, dbt_manifest=dbt)
        d3 = graph.to_d3_json()
        layers = {n["id"]: n["layer"] for n in d3["nodes"]}
        source_id = [nid for nid in layers if nid.startswith("dbt_source:")][0]
        assert layers[source_id] == 0
