...
class TestGenerateOutputContent:
    ...
    def test_index_has_lineage_view(self, tmp_path):
        runner = CliRunner()
        with runner.isolated_filesystem(temp_dir=tmp_path) as td:
            _write_manifest(Path(td))
            runner.invoke(generate_cmd, ["--output", "docs"], catch_exceptions=False)
    ...
    def test_index_has_sidebar_and_catalog(self, tmp_path):
        runner = CliRunner()
        with runner.isolated_filesystem(temp_dir=tmp_path) as td:
            _write_manifest(Path(td))
            runner.invoke(generate_cmd, ["--output", "docs"], catch_exceptions=False)
            ...
            assert "entity-tbody" in html
            assert "CATALOG_DATA" in html
