...
class TestYAMLDatabaseSchemaWarnings:
    ...
    def test_yaml_database_generates_warning(self, caplog):
        ...
        model = {
            ...
            "meta": {"sst": {"database": "ANALYTICS", "primary_key": ["id"]}},  # This should be IGNORED
        }
    ...
    def test_yaml_schema_generates_warning(self, caplog):
        ...
        model = {
            "name": "test_model",
            "description": "Test model",
            "meta": {"sst": {"schema": "MEMBERSHIPS", "primary_key": ["id"]}},  # This should be IGNORED
        }
    ...
    def test_yaml_ignored_even_with_manifest(self, caplog):
        ...
        mock_manifest.get_location.return_value = {"database": "CORRECT_DATABASE", "schema": "CORRECT_SCHEMA"}
    ...
    def test_no_warning_when_yaml_omits_database_schema(self, caplog):
        ...
        mock_manifest = Mock()
        mock_manifest.manifest = True
        mock_manifest.get_location.return_value = {"database": "ANALYTICS", "schema": "MEMBERSHIPS"}
    ...
    def test_target_database_overrides_manifest(self, caplog):
        ...
        model = {
            "name": "test_model",
            "description": "Test model",
            "meta": {"sst": {"database": "WRONG_DATABASE", "primary_key": ["id"]}},  # Should be IGNORED
        }
        ...
        result = extract_table_info(
            model, file_path, target_database="SCRATCH", manifest_parser=mock_manifest  # Override for dev environment
        )
...
class TestManifestOnlyBehavior:
    ...
    def test_without_manifest_and_without_yaml_empty_strings(self):
        ...
        model = {"name": "test_model", "description": "Test model", "meta": {"sst": {"primary_key": ["id"]}}}
