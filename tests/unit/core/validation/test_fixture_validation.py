...
class TestFixtureBasedValidation:
    ...
    def test_error_fixtures_produce_errors(self, validator, parser):
        ...
        for fixture_path in error_fixtures:
            ...
            if not any(component in str(fixture_path) for component in 
                      ['metrics', 'relationships', 'filters', 'templates']):
                continue
            ...
            try:
                ...
                assert result.has_errors or result.has_warnings, f"Expected errors/warnings for {fixture_path}, but validation passed"
                
            except Exception as e:
                # If fixture loading fails, that's also a test failure
                pytest.fail(f"Failed to test fixture {fixture_path}: {e}")
    
    @pytest.mark.skip(reason="Test fixtures need template resolution setup - will be fixed in follow-up PR")
    def test_success_fixtures_pass_validation(self, validator, parser):
        """Test that all success fixtures pass validation with template resolution."""
        success_fixtures = get_fixtures_by_severity('success')
        
        if not success_fixtures:
            pytest.skip("No success fixtures found")
        
        for fixture_path in success_fixtures:
            ...
            try:
                # Parse the fixture file with template resolution enabled
                # This simulates the real-world workflow where templates are resolved during parsing
                parse_result = parser.parse_all_files([], [fixture_path])
                
                # Add complete, valid dbt metadata for all referenced tables
                parse_result['dbt'] = {
                    'sm_tables': [
                        {
                            'table_name': 'ORDERS',
                ...
                # Validate (dbt model structure checking disabled, but table references validated)
                result = validator.validate(parse_result, check_dbt_models=False)
                
                # Should not have errors (warnings and info are OK)
                # Success fixtures should have all table references present and templates resolved
                assert not result.has_errors, f"Success fixture {fixture_path} produced errors: {[str(e) for e in result.get_errors()]}"
                
            except Exception as e:
                # If fixture loading fails, that's also a test failure
                pytest.fail(f"Failed to test success fixture {fixture_path}: {e}")
        ...
        for fixture_path in warning_fixtures:
            ...
            try:
                ...
                if not (result.has_warnings or result.has_errors):
                    ...
                    print(f"INFO: Warning fixture {fixture_path.name} didn't produce warnings - may need validation rule")
    ...
    @pytest.mark.skip(reason="Test fixtures need template resolution setup - will be fixed in follow-up PR")
    def test_valid_fixtures_pass_validation(self, validator, parser):
        """Test that all valid fixtures pass validation with template resolution."""
        valid_fixtures = get_fixtures_by_severity('valid')
        ...
        for fixture_path in valid_fixtures:
            ...
            try:
                ...
                # Should not have errors (warnings and info are OK)
                assert not result.has_errors, f"Valid fixture {fixture_path} produced errors: {[str(e) for e in result.get_errors()]}"
                
            except Exception as e:
                pytest.fail(f"Failed to test valid fixture {fixture_path}: {e}")
    ...
    def test_invalid_fixtures_produce_errors(self, validator, parser):
        ...
        for fixture_path in invalid_fixtures:
            ...
            try:
                ...
                # Should have errors or warnings
                assert result.has_errors or result.has_warnings, f"Expected errors/warnings for invalid fixture {fixture_path}, but validation passed"
    ...
    def test_edge_case_fixtures_handle_gracefully(self, validator, parser):
        ...
        for fixture_path in edge_case_fixtures:
            try:
                ...
                # Create parse result
                parse_result = {
                    'semantic': {'metrics': {'items': [fixture_data]}},
                    'dbt': {}
                }
