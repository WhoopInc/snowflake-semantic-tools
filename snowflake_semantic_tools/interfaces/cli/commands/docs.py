...
def generate_cmd(ctx: click.Context, output_dir: str, output_format: str, verbose: bool, quiet: bool) -> None:
    ...
    dbt_manifest_path = Path("target") / "manifest.json"
    dbt_manifest = None
    if dbt_manifest_path.exists():
        try:
            ...
            if not quiet:
                output.info("  dbt manifest loaded (full lineage enabled)")
        except (json.JSONDecodeError, IOError):
            pass

    graph = LineageGraphBuilder.from_manifest_data(manifest_data, dbt_manifest=dbt_manifest)
